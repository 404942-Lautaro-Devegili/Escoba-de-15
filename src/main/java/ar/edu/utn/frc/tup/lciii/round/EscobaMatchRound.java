...
public class EscobaMatchRound {
    ...
    private void solveSevenWinner() {
        ...
        Boolean appTieneTodosLosSiete = false;
        Boolean playerTieneTodosLosSiete = false;
        int contadorSietesApp = 0;
        int contadorSietesHuman = 0;
        RoundPlayer p;
        for (Card c: roundPlayerHuman.getPersonalDeck()) {
            if (c.getNumber() == 7) {
                contadorSietesHuman++;
            }
        }

        for (Card c: roundPlayerApp.getPersonalDeck()) {
            if (c.getNumber() == 7) {
                contadorSietesApp++;
            }
        }
        if (contadorSietesHuman > contadorSietesApp) {
            roundPlayerHuman.setRoundPoints(roundPlayerHuman.getRoundPoints()+1);
            if (contadorSietesHuman == 4) {
                roundPlayerHuman.setRoundPoints(roundPlayerHuman.getRoundPoints()+1);
            }
        } else if (contadorSietesApp > contadorSietesHuman){
            roundPlayerApp.setRoundPoints(roundPlayerApp.getRoundPoints()+1);
            if (contadorSietesApp == 4) {
                roundPlayerApp.setRoundPoints(roundPlayerApp.getRoundPoints()+1);
            }
        }
        return;
    ...
    private Boolean sevenOroCheck(List<Card> cards) {
        ...
        for (Card c : cards) {
            if (c.getCardSuit() == CardSuit.OROS && c.getNumber() == 7) {
                return true;
            }
        }
        return false;
