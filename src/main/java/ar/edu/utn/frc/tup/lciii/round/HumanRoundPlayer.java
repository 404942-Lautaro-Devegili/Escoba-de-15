...
public class HumanRoundPlayer extends RoundPlayer {
    ...
    private Integer getCardToPlay() {
        ...
        //   1. Mostrar este mensaje "These are the cards in your hand:"
        ...
        for (Card c : this.getHandCards()) {
            i = this.getHandCards().indexOf(c);
            cartas = cartas + i + " - " + c.getNumber() + " de " + c.getCardSuit().toString() + "\n";
        }
        print(cartas);
        ...
        while (selectedCardIndex < 0 || selectedCardIndex >= this.getHandCards().size()) {
            if (scanner.hasNextInt()) {
                ...
                if (selectedCardIndex < 0 || selectedCardIndex >= this.getHandCards().size()) {
                    System.out.println("Invalid card. Please, select a valid card.");
                }
            } else {
                System.out.println("Invalid input. Please, insert a number to get a valid option.");
